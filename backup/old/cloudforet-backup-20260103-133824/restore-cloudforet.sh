...
kubectl wait --for=condition=ready pod -l app=mongodb -n cloudforet --timeout=300s

# 4. Restore MongoDB data
echo "Restoring MongoDB data..."
MONGODB_POD=$(kubectl get pods -n cloudforet -l app=mongodb -o jsonpath='{.items[0].metadata.name}')
